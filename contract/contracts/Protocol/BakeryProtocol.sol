...
import "./BakerySwap/IBakerySwapRouter.sol";
import "./PancakeSwap/IPancakeFactory.sol";
import "./PancakeSwap/IPancakePair.sol";
import "./IDexProtocol.sol";

contract BakeryProtocol is IDexProtocol{
    using SafeMath for uint256;

    address public factory = address(0x01bF7C66c6BD861915CdaaE475042d3c4BaE16A7);
    address public _router = address(0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F);
    ...
    function safeSwapBNB(address toToken, bool withdraw) public override payable {
        ...
        IBakerySwapRouter(_router).swapExactBNBForTokens{value: amount}(0, path, address(this), now.add(600));
    ...
    function safeSwapBEP(uint256 inAmount, address fromToken, address toToken, bool withdraw) public override {
        ...
        if(toToken == BNB)
            IBakerySwapRouter(_router).swapExactTokensForBNB(amount, 0, path, address(this), now.add(600));
        else
            IBakerySwapRouter(_router).swapExactTokensForTokens(amount, 0, path, address(this), now.add(600));
